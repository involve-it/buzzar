...
bz.bus.postsHandler = {
	...
	buildPostObject: function (_post) {
		...
		if (postDb.details.photos && postDb.details.photos.length > 0) {
			post.details.photosUrls = bz.cols.images.find({_id: {$in: post.details.photos}}).fetch().map(function (p) {
				return p.data;
			});
		}
		// hz:?
			...
			_.each(posts, function (postDb) {
				...
				post = {
					...
					likes: _.filter(likes, function (like) {
						return like.entityId === postDb._id
					}).length
				};
				if (Meteor.userId()) {
					post.liked = !!_.find(likes, function (like) {
						return like.entityId === postDb._id && like.userId === Meteor.userId()
					});
				}
				_.each(postDb.details.locations, function (item) {
